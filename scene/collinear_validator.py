#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Collinear Position Validator
===========================

Detect collinearity for room objects (including the agent) to ensure no three locations fall on the same line.
Uses a 0.8-wide tolerance band to decide if points are collinear.
"""

import math
from typing import List, Dict, Tuple, Optional, Any
import numpy as np


def point_to_line_distance(px: float, py: float, 
                          x1: float, y1: float, 
                          x2: float, y2: float) -> float:
    """
    Compute the distance from a point to a line.
    """
    # Fallback to point-to-point distance when the line degenerates
    if abs(x2 - x1) < 1e-6 and abs(y2 - y1) < 1e-6:
        return math.sqrt((px - x1)**2 + (py - y1)**2)
    
    # Use |ax + by + c| / sqrt(a^2 + b^2)
    # Line equation: (y2-y1)x - (x2-x1)y + (x2-x1)y1 - (y2-y1)x1 = 0
    a = y2 - y1
    b = -(x2 - x1)
    c = (x2 - x1) * y1 - (y2 - y1) * x1
    
    distance = abs(a * px + b * py + c) / math.sqrt(a**2 + b**2)
    return distance


def are_three_points_collinear(p1: Tuple[float, float], 
                              p2: Tuple[float, float], 
                              p3: Tuple[float, float], 
                              tolerance: float = 0.5) -> bool:
    """
    Determine whether three points lie within the specified collinearity tolerance.

    Args:
        p1, p2, p3: Point coordinates (x, z).
        tolerance: Distance threshold (half of the 0.8 band).

    Returns:
        True if the points are collinear, False otherwise.
    """
    x1, z1 = p1
    x2, z2 = p2  
    x3, z3 = p3
    
    # Check each point against the line formed by the other two
    distances = [
        point_to_line_distance(x1, z1, x2, z2, x3, z3),  # p1 to line p2-p3
        point_to_line_distance(x2, z2, x1, z1, x3, z3),  # p2 to line p1-p3
        point_to_line_distance(x3, z3, x1, z1, x2, z2)   # p3 to line p1-p2
    ]
    
    # Any point within tolerance implies collinearity
    min_distance = min(distances)
    is_collinear = min_distance <= tolerance
    
    if is_collinear:
        print(f"[DEBUG] Collinearity detected: min_distance={min_distance:.3f} <= {tolerance}, "
              f"points [({x1:.1f},{z1:.1f}),({x2:.1f},{z2:.1f}),({x3:.1f},{z3:.1f})]")
    
    return is_collinear


def check_room_collinearity(positions: List[Tuple[float, float]], 
                           names: List[str],
                           tolerance: float = 0.5) -> List[Tuple[int, int, int]]:
    """
    Check whether any triplets of positions are collinear.

    Args:
        positions: List of (x, z) entries including objects and agent.
        names: Labels for debugging output.
        tolerance: Collinearity tolerance (half of the 0.8 band).

    Returns:
        List of index triplets (i, j, k) that form collinear configurations.
    """
    if len(positions) < 3:
        return []
    
    collinear_groups = []
    n = len(positions)
    
    # Check every combination of three points
    for i in range(n):
        for j in range(i + 1, n):
            for k in range(j + 1, n):
                if are_three_points_collinear(positions[i], positions[j], positions[k], tolerance):
                    collinear_groups.append((i, j, k))
                    print(f"[WARN] Collinearity found: {names[i]}({positions[i][0]:.1f},{positions[i][1]:.1f}) - "
                          f"{names[j]}({positions[j][0]:.1f},{positions[j][1]:.1f}) - "
                          f"{names[k]}({positions[k][0]:.1f},{positions[k][1]:.1f})")
    
    return collinear_groups


class CollinearValidator:
    """Validator that checks whether room objects become collinear."""
    
    def __init__(self, tolerance_width: float = 0.8):
        """
        Args:
            tolerance_width: Width of the acceptable collinearity band.
        """
        self.tolerance = tolerance_width / 2.0  # Use half as the point-to-line distance threshold
        
    def validate_room_positions(self, 
                               room_objects: List[Any],
                               agent_pos: Optional[Dict[str, float]] = None,
                               room_id: int = None,
                               room_analyzer: Any = None) -> Tuple[bool, List[Tuple[int, int, int]]]:
        """
        Validate whether any room positions become collinear.

        Args:
            room_objects: PlacedObject instances in the room.
            agent_pos: Agent position dict {"x": x, "z": z} or AgentInfo.
            room_id: Room identifier used for filtering/debugging.
            room_analyzer: Analyzer that exposes door positions.
        
        Returns:
            (is_valid, collinear_groups)
            is_valid: True when no collinear groups exist.
            collinear_groups: List of index triplets.
        """
        # Collect all positions and names
        positions = []
        names = []
        
        # Object positions
        for obj in room_objects:
            # Use final position (with default offsets)
            final_pos = obj.get_final_position()
            positions.append((final_pos["x"], final_pos["z"]))
            names.append(obj.name)
        
        # Agent position (if present and in the same room)
        if agent_pos:
            # Support both AgentInfo and dict formats
            if hasattr(agent_pos, 'room_id') and hasattr(agent_pos, 'pos'):
                # AgentInfo object
                if room_id is None or agent_pos.room_id == room_id:
                    positions.append((agent_pos.pos["x"], agent_pos.pos["z"]))
                    names.append("agent")
            elif isinstance(agent_pos, dict):
                # Dict-based payload
                if room_id is None or agent_pos.get('room_id') == room_id:
                    agent_x = agent_pos.get("x", 0.0)
                    agent_z = agent_pos.get("z", 0.0)
                    positions.append((agent_x, agent_z))
                    names.append("agent")
        
        # Include door positions linked to this room
        if room_analyzer and room_id is not None:
            door_positions_in_room = self._get_doors_for_room(room_analyzer, room_id)
            for door_name, door_pos in door_positions_in_room:
                positions.append(door_pos)
                names.append(door_name)
                print(f"[DEBUG] Added door position: {door_name} at ({door_pos[0]:.1f}, {door_pos[1]:.1f})")
        
        if len(positions) < 3:
            return True, []  # Fewer than 3 points cannot be collinear
        
        print(f"[INFO] Checking room {room_id} collinearity for {len(positions)} positions")
        
        # Detect collinearity
        collinear_groups = check_room_collinearity(positions, names, self.tolerance)
        
        is_valid = len(collinear_groups) == 0
        
        if not is_valid:
            print(f"[WARN] Room {room_id} has {len(collinear_groups)} collinear groups")
        else:
            print(f"[INFO] Room {room_id} passed collinearity validation")
        
        return is_valid, collinear_groups
    
    def _get_doors_for_room(self, room_analyzer: Any, room_id: int) -> List[Tuple[str, Tuple[float, float]]]:
        """
        Retrieve door positions connected to a specific room.
        """
        door_positions = []
        
        if not hasattr(room_analyzer, 'doors'):
            return door_positions
        
        for door_id, door_info in room_analyzer.doors.items():
            # Verify whether the door connects to the specified room
            if room_id in door_info.connected_rooms:
                door_name = f"door_{door_id}"
                door_x, door_z = door_info.center
                door_positions.append((door_name, (door_x, door_z)))
        
        return door_positions
    
    def get_repositioning_suggestions(self, 
                                    room_objects: List[Any],
                                    collinear_groups: List[Tuple[int, int, int]],
                                    room_cells: List[Tuple[int, int]],
                                    room_analyzer: Any,
                                    agent_pos: Any = None) -> List[Dict[str, Any]]:
        """
        Generate relocation suggestions for collinear objects.
        """
        suggestions = []
        
        for group_idx, (i, j, k) in enumerate(collinear_groups):
            # Collect indices corresponding to movable objects
            obj_indices = []
            for idx in [i, j, k]:
                if idx < len(room_objects):  # Ensure it's an object index
                    obj_indices.append(idx)
            
            # Skip if nothing can move
            if not obj_indices:
                continue
            
            # Move the last object in the list
            obj_to_move_idx = obj_indices[-1]
            obj_to_move = room_objects[obj_to_move_idx]
            
            # Search for alternative positions
            alternative_positions = self._find_alternative_positions(
                obj_to_move, room_objects, room_cells, room_analyzer, agent_pos
            )
            
            suggestion = {
                "group_index": group_idx,
                "object_index": obj_to_move_idx,
                "object_name": obj_to_move.name,
                "current_position": obj_to_move.get_final_position(),
                "alternative_positions": alternative_positions[:3]  # Max 3 suggestions
            }
            suggestions.append(suggestion)
        
        return suggestions
    
    def _find_alternative_positions(self, 
                                  target_obj: Any,
                                  room_objects: List[Any], 
                                  room_cells: List[Tuple[int, int]],
                                  room_analyzer: Any,
                                  agent_pos: Any = None) -> List[Dict[str, float]]:
        """Find alternative positions that reduce collinearity."""
        alternatives = []
        
        # Convert room cells to world coordinates
        for row, col in room_cells:
            world_x, world_z = room_analyzer._cell_to_world(row, col)
            int_x, int_z = int(round(world_x)), int(round(world_z))
            
            # Verify candidate is valid and avoids new issues
            if self._is_position_valid_for_relocation(
                float(int_x), float(int_z), target_obj, room_objects, room_analyzer, agent_pos
            ):
                alternatives.append({"x": float(int_x), "y": 0.05, "z": float(int_z)})
                
                if len(alternatives) >= 5:  # Limit candidate count
                    break
        
        return alternatives
    
    def _is_position_valid_for_relocation(self, 
                                        x: float, z: float,
                                        target_obj: Any,
                                        room_objects: List[Any],
                                        room_analyzer: Any,
                                        agent_pos: Any = None) -> bool:
        """Check whether relocating an object to (x, z) is valid."""
        # Ensure the position belongs to the room
        if not room_analyzer.is_position_valid_for_placement(x, z, target_obj.room_id):
            return False
        
        # Basic collision distance check
        min_distance = 1.1  # Minimum spacing
        for obj in room_objects:
            if obj.object_id != target_obj.object_id:
                final_pos = obj.get_final_position()
                obj_x, obj_z = final_pos["x"], final_pos["z"]
                distance = ((x - obj_x) ** 2 + (z - obj_z) ** 2) ** 0.5
                if distance < min_distance:
                    return False
        
        # Avoid direct overlap with other objects
        for obj in room_objects:
            if obj.object_id != target_obj.object_id:
                final_pos = obj.get_final_position()
                if abs(x - final_pos["x"]) < 0.1 and abs(z - final_pos["z"]) < 0.1:
                    return False
        
        # Keep distance from the agent
        if agent_pos:
            agent_x, agent_z = None, None
            if hasattr(agent_pos, 'room_id') and hasattr(agent_pos, 'pos'):
                if agent_pos.room_id == target_obj.room_id:
                    agent_x, agent_z = agent_pos.pos["x"], agent_pos.pos["z"]
            elif isinstance(agent_pos, dict):
                if agent_pos.get('room_id') == target_obj.room_id:
                    agent_x, agent_z = agent_pos.get("x", 0.0), agent_pos.get("z", 0.0)
            
            if agent_x is not None and agent_z is not None:
                distance = ((x - agent_x) ** 2 + (z - agent_z) ** 2) ** 0.5
                if distance < min_distance:
                    return False
        
        # Keep distance from doors
        if room_analyzer:
            door_positions_in_room = self._get_doors_for_room(room_analyzer, target_obj.room_id)
            for door_name, door_pos in door_positions_in_room:
                door_x, door_z = door_pos
                distance = ((x - door_x) ** 2 + (z - door_z) ** 2) ** 0.5
                if distance < min_distance:
                    return False
        
        # Only basic checks here; final validation occurs after applying fixes
        return True


def apply_collinear_fix(room_objects: List[Any],
                       suggestions: List[Dict[str, Any]],
                       room_analyzer: Any) -> bool:
    """
    Apply relocation suggestions to resolve collinear configurations.
    """
    fixed_count = 0
    used_positions = set()  # Track occupied candidate positions
    
    for suggestion in suggestions:
        obj_idx = suggestion["object_index"]
        alternatives = suggestion["alternative_positions"]
        
        if obj_idx < len(room_objects) and alternatives:
            obj = room_objects[obj_idx]
            
            # Choose the first unused target position
            selected_pos = None
            for alt_pos in alternatives:
                pos_key = (alt_pos["x"], alt_pos["z"])
                if pos_key not in used_positions:
                    selected_pos = alt_pos
                    used_positions.add(pos_key)
                    break
            
            if selected_pos is None:
                print(f"[WARN] No non-overlapping location available for {obj.name}; skipping fix")
                continue
            
            # Preserve previous position (supports multiple object types)
            if hasattr(obj, 'pos'):
                # True PlacedObject
                old_pos = obj.pos.copy()
                obj.pos.update(selected_pos)
            else:
                # Mock or other object implementations
                old_pos = {"x": obj.x, "z": obj.z}
                obj.x = selected_pos["x"]
                obj.z = selected_pos["z"]
            
            print(f"[INFO] Relocated {obj.name}: "
                  f"({old_pos['x']:.1f},{old_pos['z']:.1f}) -> "
                  f"({selected_pos['x']:.1f},{selected_pos['z']:.1f})")
            
            fixed_count += 1
    
    return fixed_count > 0
