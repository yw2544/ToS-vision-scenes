...
class MetadataManager:
                ...
                "objects": [
                    {
                        ...
                        "rotation": obj.rot,  # Store base rotation without default offsets
    ...
    def export_metadata(self, filename: str = "multi_room_metadata.json") -> Path:
        ...
        try:
            json_ready_metadata = self._prepare_for_json(self.metadata)
            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(json_ready_metadata, f, indent=2, ensure_ascii=False)
    ...
    def export_room_settings(self, filename: str = "room_settings.json") -> Path:
        ...
        try:
            ...
            for door_id, door_data in self.metadata["doors"].items():
                door_key = str(door_id)
                room_settings["doors"][door_key] = {
                    "name": door_data["name"],
            ...
            json_ready_settings = self._prepare_for_json(room_settings)
            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(json_ready_settings, f, indent=2, ensure_ascii=False)
    ...
    def _prepare_for_json(self, data):
        """Recursively convert metadata into JSON-safe structures."""
        if isinstance(data, dict):
            new_dict = {}
            for key, value in data.items():
                if isinstance(key, (np.integer, np.int32, np.int64)):
                    new_key = str(int(key))
                elif isinstance(key, (int, float, bool, str)) or key is None:
                    new_key = key
                else:
                    new_key = str(key)
                new_dict[new_key] = self._prepare_for_json(value)
            return new_dict
