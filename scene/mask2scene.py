...
DOOR_H = 1.4
EPS = 1e-6
DOOR_RECORD_JSON: Optional[Path] = None
DOOR_UNIFORM_SCALE = 0.12     # Uniform scaling factor; adjust if needed
DOOR_PRESERVE_MATERIALS = False  # Keep original materials (skip set_color) when True
DOOR_Y_BY_PIVOT_BOTTOM = True   # If pivot is at base use door_h/2, otherwise set False
...
WALL_MATERIAL_NAME = "plastic_vinyl_glossy_white"           # Desired wall material name
WALL_MATERIAL_LIB  = "materials_med.json"  # Material library (low/med/high)
...
FLOOR_MATERIAL_LIB  = "materials_med.json"      # Floor material library


def set_door_record_path(path: Optional[Path]):
    """
    Update the global door record JSON path so external callers (e.g., SceneGenerator)
    can override it from configuration.
    """
    global DOOR_RECORD_JSON
    if path is None:
        DOOR_RECORD_JSON = None
    else:
        DOOR_RECORD_JSON = Path(path)
...
def apply_material_to_special_primitive(ctrl: Controller,
    ...
    """
    Apply a material to primitive models from models_special.json by:
    1) Adding the material from the specified library.
    2) Replacing every submesh/material slot via set_visual_material.
    """
    # Fetch substructure from the model record
    mlib = ModelLibrarian(library=model_library)
    rec = mlib.get_record(model_name)
    sub = rec.substructure

    # Ensure the material exists and load it
    matlib = MaterialLibrarian(library=material_library)
    assert any(r.name == material_name for r in matlib.records), \
        f"[ERROR] Material {material_name} not found in {material_library}."
    cmds = [ctrl.get_add_material(material_name, library=material_library)]

    # Reset to white so previous colors don't affect textures
    cmds.append({"$type": "set_color", "id": object_id, "color": {"r":1, "g":1, "b":1, "a":1}})

    # Apply the material to every submesh/material slot
    for s in sub:
        submesh = s["name"]
        for i in range(len(s["materials"])):
            ...
            cmds.append({
                ...
                "object_name": submesh,
                "material_index": i,
                "scale": {"x": 3.5, "y": 3.5}  # Slightly adjust UV scale
            })
...
def build_scene(ctrl,
    ...
    # 1) one whole floor for each room (use room bbox, optionally shrink to wall interior surface)
    ...
    # 2) continuous walls + precise door openings (including your door model and door jamb logic)
    ...
    # return tracking information for doors and fillers (if needed by caller)
    return (door_ids, filler_ids, door_pos, filler_pos)

# ---- Segment collection for precise wall trimming ----
class SegSet:
    """Manage line segments with add/cut operations."""
    def __init__(self):
        self.segments = []  # Store (start, end) segment tuples
    ...
    def cut(self, cut_start, cut_end):
        """Remove an interval from every segment."""
        new_segments = []
...
def _is_band(v: int) -> bool:
    """check if the cell is a wall band (wall or door)"""
    return v == 0 or is_door(v)


def _nearest_room_row(mask, col, start_row, direction):
    """search for the nearest room in the specified column and direction"""
    rows = len(mask)
    r = start_row
    while 0 <= r < rows:
...
def _nearest_room_col(mask, row, start_col, direction):
    """search for the nearest room in the specified row and direction"""
    cols = len(mask[0])
    c = start_col
    while 0 <= c < cols:
...
# calculate the "continuous length" along a specific axis centered at (r,c)
# only_zero=True counts only wall cells; False counts wall bands (0 or door)
...
def _choose_orientation(mask, r, c):
    ...
    # Determine whether each direction reaches a room
    ...
    if r == 0 or r == rows - 1:
        return (True, False)      # Top/bottom rows → horizontal
    if c == 0 or c == cols - 1:
        return (False, True)      # Left/right columns → vertical

    THRESH = 3  # Threshold for three consecutive zeros

    # Prefer pure-zero runs first
    ...
    if v0 >= THRESH and h0 < THRESH:
        return (False, True)      # Stronger vertical band
    if h0 >= THRESH and v0 < THRESH:
        return (True, False)      # Stronger horizontal band
    if h0 >= THRESH and v0 >= THRESH:
        return (True, True)       # Cross intersection, allow both

    # Fall back to mixed wall-band runs (0 or door)
...
# ---- wall placement utility functions ----
def _place_wall_segments_along_z(ctrl, *, x_const, z0, z1, door_z_list, wall_t, wall_h, door_w):
    """place wall segments along z-axis, cut out precise openings at door positions"""
    EPS = 1e-6
    segs = SegSet()
    segs.add(z0, z1)
...
def _place_wall_segments_along_x(ctrl, *, z_const, x0, x1, door_x_list, wall_t, wall_h, door_w):
    """along x-axis place wall segments, cut out precise openings at door positions"""
    EPS = 1e-6
    segs = SegSet()
    segs.add(x0, x1)
...
def _add_door_visual(ctrl, *, x, z, wall_h, wall_t, door_w, horizontal, color=None):
    """
    Load a custom door from record.json when available; otherwise fall back to a colored cube.
    ...
    try:
        if DOOR_RECORD_JSON and DOOR_RECORD_JSON.exists():
            ...
            # If pivot is at the base, raise by half door height; otherwise keep at zero
            door_h = DOOR_H  # Could use wall_h, but 2.1m looks more realistic
            ...
            # Important: avoid set_color on real models to preserve materials
            ctrl.communicate(cmds)
    ...
    # ---- Fallback: use a colored cube placeholder if the custom model fails ----
    if horizontal:
...
def _build_rect_floors_by_room(ctrl: Controller,
    ...
    for rid, b in bboxes.items():
        ...
        # Optionally inset floor tiles to the inner wall face to avoid overlap
        ...
        if x_len <= 1e-6 or z_len <= 1e-6:
            continue  # Skip degenerate/thin slivers

        pos = {"x": (x0 + x1) / 2.0, "y": floor_y + half_th, "z": (z0 + z1) / 2.0}
        scale = {"x": x_len+1, "y": floor_thickness, "z": z_len+1}

        # Create a prim_cube floor (paint white before applying material)
...
def _build_continuous_walls_with_precise_doors(ctrl, mask, rows, cols, cell, wall_t, wall_h, door_w, door_colors=None):
    ...
    Returns (door_object_ids, filler_object_ids, door_positions, filler_positions) for teleportation tracking.
    """
    
    # Build direction maps
    dirH, dirV = _build_dir_maps(mask)
    used_doors = set()  # Avoid rendering duplicate door visuals
    ...
    # Generate horizontal walls (scan row by row)
    for r in range(rows):
        c = 0
        while c < cols:
            if not _is_band(mask[r][c]):
            ...
            # Consume the full wall band (doors included) while tracking whether dirH appears
            ...
            # Only emit a horizontal wall if dirH was seen within this segment
            if not seenH:
                continue
            

            # Compute world-space extent with half-cell padding at both ends
            x_const = x_center(r, cell, rows)
            # Neighboring openness (out of bounds or -1)
            ...
            # Trim endpoints if adjacent to open space or rooms
            left_trim  = left_is_open  or left_has_room
            right_trim = right_is_open or right_has_room

            # Trimmed sides inset by half a cell; otherwise extend to cover the corner
            ...
            # left_is_open  = (start_c - 1 < 0)      or (mask[r][start_c - 1] < 0)
            # right_is_open = (end_c + 1 >= cols)    or (mask[r][end_c + 1]   < 0)
            # l_ext = -0.5 if left_is_open  else (cell / 2.0)
            # r_ext = -0.5 if right_is_open else (cell / 2.0)
            ...
            # Place wall segments with exact door cutouts
            ...
            for dc in door_cols:
                if (r, dc) not in used_doors:
                    ...
                    # Track door object IDs and positions
                    if door_obj_id is not None:
                        door_object_ids.append(door_obj_id)
                        door_h = DOOR_H if DOOR_Y_BY_PIVOT_BOTTOM else 0.0  # Pivot-dependent Y placement
    ...
    for c in range(cols):
        ...
        while r < rows:
            ...
            # x0 = x_boundary(start_r, cell, rows) - cell/2
            # x1 = x_boundary(end_r + 1, cell, rows) + cell/2
            ...
            # Trim endpoints when adjacent to open space or rooms
            ...
            for dr in door_rows:
                if (dr, c) not in used_doors:
                    ...
                    # Track filler object IDs and positions
                    if filler_obj_id is not None:
                        filler_object_ids.append(filler_obj_id)
                        filler_y = FILLER_BOTTOM_Y + (wall_h - FILLER_BOTTOM_Y) / 2.0
                        filler_positions[filler_obj_id] = {"x": door_x, "y": filler_y, "z": door_z}
    ...
    # Return collected door/filler info
...
def main():
    ...
    if not args.mask_path:
        ap.error("--mask_path is required when running mask2scene.py directly")

    mask = load_mask(Path(args.mask_path))
    rows, cols = len(mask), len(mask[0])
