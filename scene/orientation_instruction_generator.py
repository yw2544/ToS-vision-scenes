...
class OrientationInstructionGenerator:
    ...
    def __init__(self, instruction_images_dir: str, output_dir: Path,
                 dual_images_dir: Optional[str] = None,
                 side_images_dir: Optional[str] = None):
        """
        ...
        self.side_dir_explicit = side_images_dir is not None
        self.dual_dir_explicit = dual_images_dir is not None
        
        if side_images_dir:
            self.side_images_dir = Path(side_images_dir)
        else:
            self.side_images_dir = Path(str(instruction_images_dir) + "_side")
        
        if dual_images_dir:
            self.dual_images_dir = Path(dual_images_dir)
        else:
        ...
        print(f"[INFO] Loaded {len(self.available_images)} front-view images")
        print(f"[INFO] Loaded {len(self.available_side_images)} side-view images")
        print(f"[INFO] Loaded {len(self.available_dual_images)} dual-view images")
        
        # Font settings for labels
        self.font_size = 48  # Larger font size for labels
    ...
    def _add_label_to_image(self, image: Image.Image, label: str) -> Image.Image:
        ...
        padding = 10  # Extra padding for readability
        label_x = padding
        label_y = padding
        
        # Draw background rectangle for better readability
        bg_padding = 8  # Extra padding for label background
        bg_rect = [
            label_x - bg_padding,
            label_y - bg_padding,
    ...
    def generate_instruction_image(self, metadata_path: Path, 
        ...
        try:
            ...
            if not objects_data:
                print("[WARNING] No valid objects found for instruction image")
                return None
            
            # Extract camera labels mapping from metadata
            camera_labels = {}
            cameras_section = metadata.get("cameras")

            def _register_camera_entry(entry: Dict[str, Any]):
                if not isinstance(entry, dict):
                    return
                camera_id = entry.get("id")
                camera_label = entry.get("label")
                if not camera_id or not camera_label or camera_id == "agent":
                    return
                try:
            ...
            if isinstance(cameras_section, list):
                for camera in cameras_section:
                    _register_camera_entry(camera)
            elif isinstance(cameras_section, dict):
                for value in cameras_section.values():
                    if isinstance(value, list):
                        for camera in value:
                            _register_camera_entry(camera)
                    else:
                        _register_camera_entry(value)
            
            print(f"[INFO] Found {len(camera_labels)} camera labels for objects")
            ...
            for obj in objects_data:
                ...
                if has_orientation:
                    ...
                    image_type_desc = "single-view image"
                ...
                if image_path:
                    ...
                    print(f"[WARNING] No matching {image_type_desc} for {model_name}{color_info} (object_id:{object_id})")
            
            print(f"[INFO] Matched {matched_count}/{len(objects_data)} objects")
            
            if not labeled_images:
                print("[WARNING] No object images were loaded successfully")
                return None
            ...
            print(f"[INFO] Orientation instruction image saved to: {output_path}")
            return output_path
